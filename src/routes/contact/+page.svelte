...
<section>
	{#if apiResult === null}
		<div in:fly={{ x: -50 }}>
			...
			<p>
				...
				<span in:fly={{ x: -50, delay: 1400 }}> let's get in touch.</span>
			</p>
		</div>
	...
	<form on:submit|preventDefault={handleSubmit} method="POST">
		...
		{/if}
		<input type="email" id="email" name="email" on:change={handleChange} bind:value={$form.email} />
		...
		<button type="submit" disabled={$isSubmitting} class="primary">Send</button>
	</form>
</section>
